...
class Index extends Component {
  ...
  handlers = {
    onSubmit: (err, values) =>{
        ...
        if(mode == 3){
                let { partName,partType=2,remark,inId,outId } = values;
                let obj = {  partName,partType,remark };
                obj.partParams =  JSON.stringify( {inId,outId} );
  ...
  render() {
    ...
    let filePartList = this._filtData.bind(this,toJS(partList),2)();
      ...
      <Card bordered={false} className="point-config">
        <Row type="flex">
            <Col span={24}>
              ...
              <Button style={{ marginRight: '16px' }}  onClick={  this._addCube.bind(this) } type="primary">增加气压表</Button>
              </div>
