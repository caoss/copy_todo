    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          render(col, row) {
            return <div>{timeFormat(col, 'yyyy-MM-dd hh:mm:ss')}</div>
          }
        },
        {
          title: '开发负责人',
          ...
          render: (id, row, index) => (
            <div>
              ...
              <Button
                ...
                onClick={() => {
                  // SystemUtils.push('/workflow')
                  const {id,resetId,autoId,versionName} = row;
                SystemUtils.push({
                    pathname: '/workflow',
                    state: {id,resetId,autoId,versionName}
                  })
                //   window.open(`/workflow?id=${row.id}&resetId=${row.resetId}&aotoId=${row.autoId}`)
                }}
              >
                流程设计
              </Button>
              <Button
                ...
                onClick={() => {
                  ...
                  // window.open(`/mainboard?id=${row.id}`)
                }}
              >
                拓扑设计
              </Button>
              ...
              >
                <a href={`http://119.23.253.207:6789/packaging/${row.id}`}>
                  打包
                </a>
              </Button>
            </div>
