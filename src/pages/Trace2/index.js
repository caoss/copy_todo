      ...
      columns: [
        {
          ...
          dataIndex: 'pointName',
          render: (id, row, index) => (
            <div style={{cursor:"pointer",padding:'20px'}} onClick={self._getPointPosition.bind(self,row.pointId)}>
                {row.pointName}
            </div>
          )
        },
...
_getPointPosition(id){
    ...
    new Promise((r, j) => {
        ...
        const { mode,mode2, data } = this.state
          if (mode === 1) {
            ...
            let { axisId,pointId,position } = obj;
            mainboardStore
              .editAxisPoint({ axisId,pointId,position })
              .then(res => {
                if (res.code === '0') {
                    this._getPointList();
            ...
            const traceList = traceStore.traceList
            traceStore
            .editTraceOffset(values,traceList[this.state.actTraceIndex].id)
            .then(res => {
              if (res && res.code === '0') {
                  this.setState({ mode: 0 });
                  this.getTraceList();
  ...
  handles2={
    ...
    _review(){
        const { traceStore } = this.props.store
        const traceList = traceStore.traceList
        let canvasData = toJS(traceList)[this.state.actTraceIndex]
        let id  = canvasData.id;
        traceStore.getPos(id).then(res=>{
            console.log(res);
            this.setState({
                posData:res,
                previewModal:1
  ...
  render() {
      let self = this;
    const { mainboardStore,traceStore } = this.props.store
    const { formData_offset, formData_p,formData3,formData2,columns, mode,mode2, data, pagination,disabledDel } = this.state
    const pointData = mainboardStore.pointData
    const traceList = traceStore.traceList

    const paginationData = Object.assign({}, pagination, {
      total: pointData[this.state.actTraceIndex]&& pointData[this.state.actTraceIndex]['pointList'] ? pointData[this.state.actTraceIndex]['pointList'].length:0,
        ...
        <Row type="flex">
          ...
          <Col span={7}>
            <div>
                ...
                <Button style={{ marginBottom: '16px',marginRight:'16px' }}  onClick={() => {
                    this.setState({ visible2: 1 })
                  }} type="primary">DXF转制</Button>
            ...
            <Col span={17}>
              ...
              <Button style={{ marginRight: '16px' }}  onClick={  this._traceOffset.bind(this) } type="primary">轨迹偏移</Button>
              <Button style={{ marginRight: '16px' }}  onClick={  this._review.bind(this) } type="primary">预览</Button>
              </div>
            ...
            width={850}
            onOk={this._callBackTrack.bind(this)}
            ...
            >
              ...
              <ModalCustomForm
                ...
                formData={ mode =='3'?formData3:mode==1||mode==4?formData2:mode==5?formData_p: mode==6? formData_offset:formData3 }
                defaultData={data}
              />
            ...
            <Modal
                ...
                onOk={this.handlePreviewOk.bind(this)}
                onCancel={this.handlePreviewCancel.bind(this)}
                >
                <TraceCanvas data={this.state.posData}/>
            </Modal>
        </Row>
      </Card>
    )
