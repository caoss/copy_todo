...
import img14 from './img/13.png';
import img15 from './img/13.png';
import img16 from './img/13.png';
import img17 from './img/13.png';
import img18 from './img/13.png';
import img19 from './img/13.png';
...
class Index extends Component {
    ...
    _getModuleGroupList(){
        ...
        return new Promise(function (resolve, reject) {
            moduleGroupStore.getList().then(res=>{
                if(res.length>0){
                    let item = res[0];
                    self._checkID( item.autoId,item.axisIds,item.partIds,1,item.mgName  );
                }
                resolve(res)
            });
        });
    ...
    getStepList(){
        ...
        const { mainboardStore } = this.props.store
        mainboardStore.getStepList({ softwareId,parentId:actionId}).then(res=>{
            this.setState({
                stepList:res
            })
        });
    ...
    handlers = {
        ...
        _submit(parms){
            ...
            }else if( obj.stepType ==20){
                obj.extParams =  JSON.stringify({traceId:obj.traceId,glueValveOutId:obj.glueValveOutId});
            }
...
getCriteria(){//操作完成之后
    ...
    if( getParentSMsg && getParentSMsg.id ){
        let { id,index,i,i3,type,i4 } = getParentSMsg;
        if(type==2){
            ...
            this._expAction4(id,index,i3,i,i4);
        }
    }else{
        this.getStepList();
    }
...
_shrinkAction(index){
    ...
    this.setState({
        stepList:mainboardStore.stepList
    })
}
...
//获取第五级信息
_expAction4(id,index,i,i3,i4){
    const { mainboardStore } = this.props.store
    const  softwareId  = this.state.id;
    mainboardStore.getStepList_5({ softwareId,parentId:id,index,i,i3,i4}).then((res)=>{
        this.setState({
            stepList:res
        })
    });
}
_shrinkAction4(index,i,i3,i4){
    const { mainboardStore } = this.props.store
    mainboardStore._shrinkAction4({index,i,i3,i4});
    this.setState({
        stepList:mainboardStore.stepList
    })
}
...
_changeEnable(parms){
    const { mainboardStore } = this.props.store
    mainboardStore.setEnable(parms).then(res=>{
        this.setState({
            stepList:mainboardStore.stepList
        })
    });
...
_checkID(id,selAxisList,partsList,type,name){
        this.setState({
            actionId:id,
            selAxisList:JSON.parse(selAxisList?selAxisList:"[]"),
            partsList:JSON.parse(partsList?partsList:"[]"),
    ...
    onDragEnd (result) {
        ...
        if(result.draggableId && result.destination){
            mainboardStore._step_conf({execOrder:result.destination['index']-0+1,id:result.draggableId}).then(
                res=>{
                    console.log(res);
                }
            )
  ...
  render() {
    ...
    const { mainboardStore,moduleGroupStore,traceStore } = this.props.store
    let  stepList = this.state.stepList;
    let modeleList = moduleGroupStore.modeleList;

    return (
            <div className='workflow_main'>
                <div className={ this.state.btn_toggle? "workflow_left":"workflow_left act" }>
                    {
                        modeleList.map((item)=>{
                            return(
                                <div>
                    ...
                    */}
                    ...
                    <Popconfirm
                                                                ...
                                                                <div>
                                                                    ...
                                                                    key={item.id}>
                                                                        <div className='step_li'>
                                                                            ...
                                                                            {
                                                                                item.enableFlag?
                                                                                    <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :0,id:item.id,i1:index})} type='primary' >
                                                                                        关闭使能
                                                                                    </Button> 
                                                                                :
                                                                                    <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :1,id:item.id,i1:index})} type='primary' >
                                                                                        使能
                                                                                    </Button> 
                                                                            }
                                                                                    ...
                                                                                    item.steps.map( (step_li,step_i)=>{
                                                                                        return(
                                                                                            <div className='li2' key={ step_i }>
                                                                                                ...
                                                                                                {
                                                                                                    step_li.enableFlag?
                                                                                                        <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :0,id:step_li.id,i1:index,i2:step_i})} type='primary' >
                                                                                                            关闭使能
                                                                                                        </Button> 
                                                                                                    :
                                                                                                        <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :1,id:step_li.id,i1:index,i2:step_i})} type='primary' >
                                                                                                            使能
                                                                                                        </Button> 
                                                                                                }
                                                                                                            ...
                                                                                                            step_li.steps.map( (li3,i3)=>{
                                                                                                                return(
                                                                                                                    <div className='li2' key={ i3 }>
                                                                                                                        ...
                                                                                                                        {
                                                                                                                            li3.enableFlag?
                                                                                                                                <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :0,id:li3.id,i1:index,i2:step_i,i3})} type='primary' >
                                                                                                                                    关闭使能
                                                                                                                                </Button> 
                                                                                                                            :
                                                                                                                                <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :1,id:li3.id,i1:index,i2:step_i,i3})} type='primary' >
                                                                                                                                    使能
                                                                                                                                </Button> 
                                                                                                                        }
                                                                                                                                            ...
                                                                                                                                            <div className='li2' key={ i4 }>
                                                                                                                                                ...
                                                                                                                                                </Button> 
                                                                                                                                                {
                                                                                                                                                    li4.stepType == 1 || li4.stepType == 2?
                                                                                                                                                        <span>
                                                                                                                                                        <Button className='icon_edit' onClick={ self._addCondition.bind(self,{ parentId:li4.id },{ id:li4.id,index,type:5,i3:step_i,i:i3,i4:i4 }) }  icon="plus" >
                                                                                                                                                            动作
                                                                                                                                                        </Button>
                                                                                                                                                        {/* 展开 */}
                                                                                                                                                        {
                                                                                                                                                            li4.expand?
                                                                                                                                                            <Button className='icon_edit' onClick={ self._shrinkAction4.bind(self,index,step_i,i3,i4 ) }  icon="shrink" />
                                                                                                                                                            :
                                                                                                                                                            <Button className='icon_edit' onClick={ self._expAction4.bind(self, li4.id, index, step_i, i3,i4) }  icon="arrows-alt" />
                                                                                                                                                        }
                                                                                                                                                        </span>
                                                                                                                                                    :
                                                                                                                                                    null
                                                                                                                                                ...
                                                                                                                                                {
                                                                                                                                                    li4.enableFlag?
                                                                                                                                                        <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :0,id:li4.id,i1:index,i2:step_i,i3,i4})} type='primary' >
                                                                                                                                                            关闭使能
                                                                                                                                                        </Button> 
                                                                                                                                                    :
                                                                                                                                                        <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :1,id:li4.id,i1:index,i2:step_i,i3,i4})} type='primary' >
                                                                                                                                                            使能
                                                                                                                                                        </Button> 
                                                                                                                                                }
                                                                                                                                                    ...
                                                                                                                                                    {/*五级*/}
                                                                                                                                                    <div className={ li4.expand?'':'hide' }>
                                                                                                                                                        {
                                                                                                                                                            li4.steps&& li4.steps.length>0?
                                                                                                                                                                li4.steps.map((li5,i5)=>{
                                                                                                                                                                    return(
                                                                                                                                                                        <div className='li2' key={ i5 }>
                                                                                                                                                                            <img className='actionIcon' src={this.imgs[li5.stepType-1] } />
                                                                                                                                                                            —— { li5.stepName?li5.stepName:'没有名字' }
                                                                                                                                                                            <Button className='icon_edit'  icon="form" onClick={ self._edit.bind(self, toJS(li5),{ id:li4.id,index,type:5,i3:step_i,i:i3,i4:i4}) }  /> 
                                                                                                                                                                            <Button className='icon_edit'  icon="plus" onClick={ self._addCriteria.bind(self, toJS(li5),{ id:li4.id,index,type:5,i3:step_i,i:i3,i4:i4}) }   >
                                                                                                                                                                                条件
                                                                                                                                                                            </Button> 
                                                                                                                                                                            ...
                                                                                                                                                                            <Popconfirm
                                                                                                                                                                                ...
                                                                                                                                                                                okText="Yes"
                                                                                                                                                                                cancelText="No"
                                                                                                                                                                                onConfirm={self._delStep.bind(self,li5.id,{ id:li4.id,index,type:5,i3:step_i,i:i3,i4:i4})}
                                                                                                                                                                            >
                                                                                                                                                                                <Button className='icon_edit' type="danger"  icon="delete"/> 
                                                                                                                                                                            </Popconfirm>

                                                                                                                                                                            <Button  onClick={self.step_config.bind(self,li5.id)}  className='icon_carrary' type='primary' >
                                                                                                                                                                                调试
                                                                                                                                                                            </Button>

                                                                                                                                                                            {
                                                                                                                                                                                li5.enableFlag?
                                                                                                                                                                                    <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :0,id:li5.id,i1:index,i2:step_i,i3,i4,i5})} type='primary' >
                                                                                                                                                                                        关闭使能
                                                                                                                                                                                    </Button> 
                                                                                                                                                                                :
                                                                                                                                                                                    <Button className='icon_clo' onClick={self._changeEnable.bind(self,{enableFlag :1,id:li5.id,i1:index,i2:step_i,i3,i4,i5})} type='primary' >
                                                                                                                                                                                        使能
                                                                                                                                                                                    </Button> 
                                                                                                                                                                            }
                                                                                                                                                                                ...
                                                                                                                                                                                <ul className='criteria'>
                                                                                                                                                                                    {
                                                                                                                                                                                        li5.criteriaList && li5.criteriaList.length>0?
                                                                                                                                                                                            li5.criteriaList.map( (cr_li,i)=>{

                                                                                                                                                                                                return(
                                                                                                                                                                                                    <li className='li2' key={ i }>
                                                                                                                                                                                                        <img className='actionIcon' src={this.imgs_c[cr_li.criteriaType-1] } />
                                                                                                                                                                                                        —— { cr_li.criteriaName }
                                                                                                                                                                                                        <Button onClick={ self._editCriteria.bind(self,toJS(cr_li),{ id:li4.id,index,type:5,i3:step_i,i:i3,i4:i4})}   size='small' className='icon_edit'  icon="form" /> 
                                                                                                                                                                                                        <Popconfirm
                                                                                                                                                                                                            title="确定删除此条件?"
                                                                                                                                                                                                            okText="Yes"
                                                                                                                                                                                                            cancelText="No"
                                                                                                                                                                                                            onConfirm={  self._delCriteria.bind(self, toJS(cr_li),{ id:li4.id,index,type:5,i3:step_i,i:i3,i4:i4})  }
                                                                                                                                                                                                        >
                                                                                                                                                                                                            <Button className='icon_edit' type="danger"  icon="delete"/> 
                                                                                                                                                                                                        </Popconfirm>
                                                                                                                                                                                                    </li>
                        ...
                        <ModalCustomForm 
                            ...
                            traceList={ toJS(traceStore.traceList) }
                            destroyOnClose
                        />
