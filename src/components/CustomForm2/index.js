...
import { Form, Input, Select, InputNumber, Switch, Col, Button, Row } from 'antd'
import './index.scss'
...
class CustomForm extends Component {
    ...
    return (
      <Col key={`${valueKey}`} span={span || 24} style={{"padding":'0 20px'}}>
        <Form.Item
          label={label}
          help={help}
          extra={extra}
        ...
        return (
            <Form onSubmit={this.handlers.onSubmit} {...rest}>

                {
                    ...
                    <div>
                        <Row>
                            <h1 className="title_axis_parm">基础参数</h1>
                            { data[0]&&data[0].length>0&& data[0].map(formItem => this.getForm(form, formItem, rest.layout)) }
                            <div style={{ color: 'transparent' }}>_</div>
                        </Row>
                        <Row>
                            <h1  className="title_axis_parm">运动参数</h1>
                            { data[1]&&data[1].length>0&& data[1].map(formItem => this.getForm(form, formItem, rest.layout)) }
                            <div style={{ color: 'transparent' }}>_</div>
                        </Row>
                        <Row>
                            <h1  className="title_axis_parm">其它</h1>
                            { data[2]&&data[2].length>0&& data[2].map(formItem => this.getForm(form, formItem, rest.layout)) }
                            <div style={{ color: 'transparent' }}>_</div>
                        </Row>
                    </div>
                    :
